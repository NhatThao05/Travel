     ...
     <div className="absolute top-0 w-full h-full flex flex-col justify-between p-1 pr-2 pt-2 pb-2 z-50" style={{height: '100%'}}>
       <div className="flex items-center justify-between">
        ...
        {alreadySave?.length !== 0 ? (
         <button className="bg-red-500 text-white opacity-70 hover:opacity-100 font-bold px-5 py-1 text-base rounded-3xl hover:shadow-md outline-none" type="button">
          {save?.length} <MdOutlineFavorite fontSize={25}/>
         </button>
        ) : (
         <button 
          onClick={(e) => {
